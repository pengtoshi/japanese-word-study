    ...
    const handleScroll = () => {
      if (headerRef.current) {
        ...
        const isStickyActive =
          Math.abs(rect.top - expectedStickyTop) < tolerance;
  ...
  async function handlePlayAudio(item: VocabItemRow) {
    ...
    try {
      const res = await fetch(
        `/api/tts/vocab?itemId=${encodeURIComponent(item.id)}`,
        {
          method: "POST",
        },
      );
  ...
  return (
    <div>
      <div
        ref={headerRef}
        className={cn(
          "sticky z-30 pt-0 pb-4 transition-all duration-300",
          "top-[calc(env(safe-area-inset-top)+65px)] px-4 pt-4 sm:px-6 sm:pt-6",
          isScrolled ? "bg-(--surface) border-b border-(--border)" : "",
        )}
      >
                      ...
                      {it.ja_surface_ruby_html ? (
                        <span
                          ...
                          dangerouslySetInnerHTML={{
                            __html: it.ja_surface_ruby_html,
                          }}
                        />
                      ) : (
                        <>
